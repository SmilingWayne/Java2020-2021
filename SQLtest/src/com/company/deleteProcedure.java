...
public class deleteProcedure {
    ...
    public static void deletePro(){
        ...
        if(choice == 1){
            ...
            System.out.println("以下是退货订单：");
            tableSelect ts = new tableSelect();
            ts.select("7");
            System.out.println("输入要删除的退货单号:");
            add = scan.next();
        }
        else if(choice == 3){
            System.out.println("以下是销售订单：");
            tableSelect ts = new tableSelect();
            ts.select("9");
            System.out.println("输入要删除的销售单号:");
            add = scan.next();
        }
        else{
            System.out.println("以下是所有图书：");
            tableSelect ts = new tableSelect();
            ts.select("4");
