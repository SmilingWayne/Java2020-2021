...
public class Main {
    public static void main(String[] args) {
        File file = new File("src/huaweiTest/training");
        try {
            ...
            Host[] hostsOnCore = hosts;
            Arrays.sort(hostsOnCore, new Comparator<Host>() {
                @Override
            ...
            //System.out.println("Host is over");
            Host[] hostsOnMemory = hosts;
            Arrays.sort(hostsOnMemory, new Comparator<Host>() {
                @Override
                public int compare(Host o1, Host o2) {
         ...
         */
                    ...
                    if (requestVM[0].equals("add")) {
                        ...
                        if (targetVM[2] == 1) {
                            ...
                            if (location != -1) {
                                ...
                                allVM.put(orderNum, new tempRecord(location, "AB", target));
                            } else {
                                String addedServer = "";
                                ...
                                totalMoney += (detailed[2] + detailed[3] *(totalDays - j));
                                ...
                                allVM.put(orderNum, new tempRecord(use.size(), "AB", target));
                            ...
                            if (singleTemp[0] == -1 && singleTemp[1] == -1) {
                                ...
                                usedHost appendix = new usedHost(addedTemp, detailed[0] - targetVM[0], detailed[0],
                                        detailed[1] - targetVM[1], detailed[1]);
                                allVM.put(orderNum, new tempRecord(use.size(), "A", target));
                                use.put(use.size(), appendix);
                            } else {
                                int destination = singleTemp[0];
                                if (singleTemp[1] == 1) {
                                    allVM.put(orderNum, new tempRecord(destination, "A", target));
                                    use.get(destination).leftRestCore -= targetVM[0];
                                    use.get(destination).leftRestMemory -= targetVM[1];
                                    use.get(destination).containVm.add(targetNum);
                                    use.get(destination).direction = 1;
                                } else if (singleTemp[1] == 2) {
                                    allVM.put(orderNum, new tempRecord(destination, "B", target));
                        ...
                        for (int k = 0; k < use.size(); k++) {
                            ...
                            if (use.get(k).containVm.contains(deleteNum)) {
                                if (vmMap.get(allVM.get(deleteNum).type)[2] == 1) {
                                    int halfMemory = vmMap.get(allVM.get(deleteNum).type)[1] / 2;
                                    int halfCore = vmMap.get(allVM.get(deleteNum).type)[0] / 2;
                                    ...
                                    if (use.get(k).direction == 1) {
                                        use.get(k).leftRestCore += vmMap.get(allVM.get(deleteNum).type)[0];
                                        use.get(k).leftRestMemory += vmMap.get(allVM.get(deleteNum).type)[1];
                                    } else if (use.get(k).direction == 2) {
                                        use.get(k).rightRestCore += vmMap.get(allVM.get(deleteNum).type)[0];
                                        use.get(k).rightRestMemory += vmMap.get(allVM.get(deleteNum).type)[1];
                                    }
                                }
                                break;
                            }
            ...
            System.out.println("This will cost " + (timeEnd - timeStart)/(1e9) + "s");
            System.out.printf("The total cost is " + bg);
            for(int i = 0 ; i < hostNum; i ++ ){
                System.out.println(" Core: " + hostsOnCore[i].hostName);
                System.out.println(" Memory: " + hostsOnMemory[i].hostName);
            }
...
class tempRecord{
    ...
    public String type;
    public tempRecord(int number, String location,String type){
        this.number = number;
